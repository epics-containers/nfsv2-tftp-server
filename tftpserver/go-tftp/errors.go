...
func (e *errUnexpectedDatagram) Error() string {
	...
	log.Printf("unexpected datagram: %s", e.dg)
...
func IsUnexpectedDatagram(err error) bool {
	...
	logfile, _ := os.OpenFile("log.txt", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0666)
	log.SetOutput(logfile)
	log.Println("IsUnexpectedDatagram==true")
...
func (e *errParsingOption) Error() string {
	logfile, _ := os.OpenFile("log.txt", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0666)
	log.SetOutput(logfile)
	log.Printf("error parsing %q for option %q", e.value, e.option)
